...
import 'package:services/marketplace/lib/api/QuincaillerieController.dart';
...
class QuicailleryBox extends StatefulWidget {
  QuicailleryBox(
      {Key key,
        this.quincailleries,
        this.data,
  ...
  HashSet<String> selectedDistricts = new HashSet(),
      selectedQuincaillery = new HashSet();
  List quincailleries;
  var data;
  GlobalKey<ScaffoldState> scaffoldKey = new GlobalKey<ScaffoldState>();

  @override
  _QuicailleryBoxBoxState createState() => new _QuicailleryBoxBoxState();
}

class _QuicailleryBoxBoxState extends State<QuicailleryBox> {
  var _formKey = GlobalKey<FormState>();
  String _current = null,
      _current1 = null,
      _current2 = null;
  int _id, quincailleryId;
  ...
  void initState() {
    ...
    widget.selectedQuincaillery.add("aucune quincaillerie");
  }

  Quincailleries() {
    widget.cities.clear();
    QuicaillerieController().getList((List<dynamic> json) {
      json.forEach((city) => {
        widget.cities.add(city["merchant"]['address']),
        widget.quincailleries.add(city["merchant"]),
        widget.listOfServices.add(CommonServiceItem.fromJson(city))
      });
      LinkedHashSet<String>.from(widget.cities).toList();
    }, (ServerResponseValidator validator) {
      print(validator.isError());
      SysSnackBar().error(widget.scaffoldKey);
    }, (ServerResponseValidator validator) {
      print(validator.isError());
    }).catchError((error) {
      widget.selectedDistricts.clear();
      setState(() {
        showInSnackBar("Aucune ville repertoriée");
      });
    });
  }
  ...
  getDistricts(String ville) {
    widget.selectedDistricts.clear();
    widget.selectedQuincaillery.clear();
    for (int i = 0; i < widget.quincailleries.length; i++) {
      if (widget.quincailleries[i]['address'] == "$ville")
        widget.selectedDistricts.add(widget.quincailleries[i]['neighborhood']);
    }
  }

  getQuincailleries(String ville, String quartier) {
    widget.selectedQuincaillery.clear();
    for (int i = 0; i < widget.data.length; i++) {
      if (widget.quincailleries[i]['address'] == "$ville" &&
          widget.quincailleries[i]['neighborhood'] == "$quartier") {
        widget.selectedQuincaillery.add(widget.quincailleries[i]['name']);
        quincailleryId = widget.quincailleries[i]['id'];
        _id = widget.quincailleries[i]['id'];
        AppState.putInt(Data.MERCHANT_ID, quincailleryId);
        AppState.putInt(Data.SELLABLE_ITEM, quincailleryId);
        getService(widget.quincailleries[i]);
        break;
      }
    }
  }
  ...
  int getId() {
    for (int i = 0; i < widget.data.length; i++) {
      if (widget.data[i]['merchant']['id'] == quincailleryId) {
                      ...
                      child: DropdownButton<String>(
                        ...
                        onChanged: (String selected) {
                          setState(() {
                            _current1 = selected;
                            _current2 = null;
                            this.getQuincailleries(_current, _current1);
                          });
                        },
                        value: null,
                        hint: Padding(
                  ...
                  child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        hint: Padding(
                          padding: EdgeInsets.only(left: 20),
                          child: Text(
                            _current2 == null ? "Votre quicaillerie" : _current2,
                            style: TextStyle(
                              color: couleur_libelle_champ,
                              fontSize: taille_libelle_champ,
                            ),
                        ...
                        items: listSort(widget.selectedQuincaillery.toList())
                            .map((String name) {
                          return DropdownMenuItem<String>(
                            value: name,
                            child: Padding(
                            ...
                            onTap: () {
                              setState(() {
                                ...
                                if (_current == null &&
                                    ...
                                    _current2 == null) {
                                  showInSnackBar(
                                      "Veuillez sélectionner une ville, un quartier ainsi qu'une quincaillerie");
                                  stopSnackBar();
                                } else if (_current != null &&
                                    _current1 == null &&
                                    _current2 == null) {
                                  this.showInSnackBar(
                                      "Veuillez sélectionner un quartier ainsi qu'une quincaillerie");
                                  stopSnackBar();
                                } else if (_current != null &&
                                    _current1 != null &&
                                    _current2 == null) {
                                  this.showInSnackBar(
                                      "Veuillez sélectionner une quincaillerie");
                                  stopSnackBar();
                                } else if (_current == null ||
                                    _current1 == null ||
                                    _current2 == null) {
