...
class Offrirtontine1 extends StatefulWidget {
  Offrirtontine1(this._codetontine);
  String _codetontine;
  @override
  _Offrirtontine1State createState() => new _Offrirtontine1State(_codetontine);
}

class _Offrirtontine1State extends State<Offrirtontine1> {
  _Offrirtontine1State(this._codetontine);
  String _codetontine;
  ...
  String kittyImage,previsional_amount,amount_collected,remain, firstnameBenef,kittyId, endDate,particip, startDate, title, suggested_amount, amount, description, number, nom="", tel="", email="", montant="", mot="";
  ...
  void initState() {
    ...
    kittyImage = _codetontine.split('^')[1];
    firstnameBenef = _codetontine.split('^')[2];
    endDate = _codetontine.split('^')[3];
    startDate = _codetontine.split('^')[4];
    title = _codetontine.split('^')[5];
    previsional_amount = _codetontine.split('^')[6]; //previsional_amount
    amount_collected = _codetontine.split('^')[7]; //amount_collected
    description = _codetontine.split('^')[8];
    number = _codetontine.split('^')[9];
    kittyId = _codetontine.split('^')[10];
    particip = _codetontine.split('^')[11];
    remain = _codetontine.split('^')[13];
  }
              ...
              child: Stack(
                children: <Widget>[
                    ...
                    child: Row(
                      children: <Widget>[
                        InkWell(
                            onTap: (){
                              setState(() {
                                Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => Detail(_codetontine)));
                                //Navigator.of(context).push(SlideLeftRoute(enterWidget: Offrir3(_code), oldWidget: Offrir1(_code)));
                              });
                            },
                  ...
                  Padding(
                    ...
                    child: Text('Quel montant vous devez offrir?',
                      style: TextStyle(
                          color: couleur_libelle_champ,
                          fontSize: taille_champ+3,
                          fontWeight: FontWeight.bold
                    ...
                    child: InkWell(
                      onTap: (){
                        ...
                        if(_formKey.currentState.validate()){
                          if(int.parse(montant.replaceAll(".", ""))<=double.parse(remain)){
                            this._save(montant);
                            Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => Paysb('$_codetontine')));
                          }else{
                            this._ackAlert(context, "Le montant à encaisser doit être inférieur à $remain");
                          }
                        }
                      },
                      child: Container(
  ...
  void _save(String valeur) async {
    ...
    prefs.setString(MONTANT_OFFRE, valeur);
  }
