...
import 'getCode.dart';
import 'ioswebview.dart';


class Paiement2 extends StatefulWidget {
  Paiement2(code);
  String code;
  @override
  _Paiement2State createState() => _Paiement2State(code);
}

class _Paiement2State extends State<Paiement2> {
  _Paiement2State(_code);
  String _code, kittyImage, kittyId, _id, payment_url, _token, country, url, title,_username,_user, montant, prenom, nom, email, description, mobileNumber, _url, currency, currencySymbol, _tel, flagUri, dial_code;
  ...
  lire() async {
    ...
    setState(() {
      ...
      if(_tel == "null" || _tel == null)
        _tel = _username;
      _user = prefs.getString(USER_PART);
      ...
      if(indik == 1 || indik == 2){
        url = "$cagnotte_url/cashin/mobilepayment";
      }else if(indik == 3){
        url = "$cagnotte_url/cashin/walletpayment/init";
      }else{
        url = "$cagnotte_url/cashin/paypalpayment";
      }
      currency = prefs.getString(CURRENCY);
                ...
                Padding(
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      child: new Center(
                        child:isLoading==false?new Text('Valider ma contribution', style: new TextStyle(color: couleur_text_bouton),):
                        Theme(
  ...
  Future<String> contribKitty(var body) async {
    print("Mon url: $url");
    ...
    if (response.statusCode < 200 || json == null) {
      ...
      if(payment_url != null){
        ...
        save();
        if(Platform.isIOS){
          Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => new IosWebview("$_code^c")));
        }else
          Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => new Webview("$_code^c")));
      }else{
        if(indik == 3){
          setState(() {
            isLoading = false;
    ...
    if(response.statusCode == 200){
      ...
      if(responseJson['status'] == "CREATED"){
        if(temps == 0){
          ...
          save();
          Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => new Echec('participation')));
        }else if(temps > 0){
          temps--;
          getStatus(id);
        }
        ...
        save();
        Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => new Confirm('participation')));
