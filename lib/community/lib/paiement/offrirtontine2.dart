...
class _Offrir2State extends State<Offrir2> {
  ...
  String kittyImage,_xaf,_token,country, previsional_amount,_username, phone, kittyId,particip, amount_collected, firstnameBenef, endDate, startDate, title, suggested_amount, amount, description, number, nom="", tel="", email="", montant="", mot="";
                          ...
                          child: Row(
                            children: <Widget>[
                              Expanded(
                                ...
                                child: Align(
                                  alignment: Alignment.centerLeft,
                                  child: Text('Frais de la transaction',
                                    style: TextStyle(
                                        color: couleur_libelle_champ,
                                        fontSize: taille_champ+3,
                                        fontWeight: FontWeight.bold
  ...
  Future<String> getOffre(var body) async {
    ...
    return await http.post(url, body: body, headers: _header, encoding: Encoding.getByName("utf-8")).then((http.Response response) {
      ...
      else Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => Echec('$_code^§')));
  ...
  Future<String> getStatus(String id) async {
    ...
    if(response.statusCode == 200){
      ...
      if(responseJson['status'] == "CREATED"){
        ...
        //Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => new Confirme('part')));
