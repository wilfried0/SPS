...
//_code = '$index ${_cagnottes[index]["kittyImage"]} ${_cagnottes[index]["firstnameBenef"]} ${_cagnottes[index]["endDate"]} ${_cagnottes[index]["startDate"]} ${_cagnottes[index]["title"]} ${_cagnottes[index]["suggested_amount"]} ${_cagnottes[index]["amount"]} ${_cagnottes[index]["description"]}';
  ...
  Future<String> createCard(var body) async {
    if(choice == 0){
      ...
      url = "$base_url/walletpayment/cashout";
        ...
        }else if(response.statusCode == 200 || response.statusCode == 403){
          var convertDataToJson = json.decode(reply)['body'];
          setState(() {
            isLoding = false;
            //this._save(null);
          });
  ...
  _getStatus(String url) async {
    ...
    print(response.statusCode);
    print(reply);
    if(json.decode(reply)['status'] == "PROCESSED"){
      monstatus = "PROCESSED";
      setState(() {
        isLoding = false;
        this._save(null);
      });
      Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => Confirmation(json.decode(reply)['transactionID'])));
      //Navigator.of(context).push(SlideLeftRoute(enterWidget: Confirmation(''), oldWidget: Verification(_code)));
    }else if(json.decode(reply)['status'] == "REFUSED"){
      if(monstatus == "PROCESSED"){
        ...
        showInSnackBar(json.decode(reply)['status']);
        Timer(Duration(seconds: 20), onDoneLoading);
      }
    }else if(json.decode(reply)['status'] == "CREATED"){
      _getStatus(momo_url);
    }
  }
