...
class Participertontine extends StatefulWidget {
  Participertontine(this._code);
  String _code;
  @override
  _ParticipertontineState createState() => new _ParticipertontineState(_code);
}

class _ParticipertontineState extends State<Participertontine> {
  _ParticipertontineState(this._code);
  String _code;
            ...
            child: Stack(
              children: <Widget>[
                ...
                Padding(
                  ...
                  child: InkWell(
                    onTap: (){
                      print("startDate:   ${startDate.toString().split("T")[0]}");
                      print("aujourd'hui:   ${DateTime.parse(DateTime.now().toString().split(" ")[0])}");
                      print("La différence vaut:   ${((DateTime.parse(startDate.toString().split("T")[0])).difference(DateTime.parse(DateTime.now().toString().split(" ")[0]))).inDays}");
                      if(((DateTime.parse(startDate.toString().split("T")[0])).difference(DateTime.parse(DateTime.now().toString().split(" ")[0]))).inDays>=0)
                        JeVeuxetreMemnre();
                      else
                        showInSnackBar("Vous ne pouvez plus adhérer à cette tontine", _scaffoldKey, 5);
                    },
                    child: Container(
                      height: hauteur_bouton,
                      width: MediaQuery.of(context).size.width,
                      ...
                      child: new Center(child: new Text('Je souhaite être membre', style: new TextStyle(color: couleur_text_bouton),),),
                    ),
                  ),
                ),
              ],
